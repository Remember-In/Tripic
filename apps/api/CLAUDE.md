...
- **GPS 좌표·EXIF 원본을 수신/저장하는 코드를 만들지 않는다.** 좌표는 단말에서만 쓰고 서버로 보내지
  않는다 — "사업자의 위치정보시스템으로 전송하지 않는다"가 위치정보법 신고 불요 판단의 전제다
  (docs/12-location-law.md §3-①②). 프록시에 좌표 파라미터를 추가하는 것도 여기에 해당한다.
- **KTO 원천 데이터를 저장하지 않는다.** 웹은 브라우저 CORS 때문에 TourAPI를 직접 호출할 수 없어
  `/tourism` 서버 프록시를 두는데, 이것은 순수 pass-through 다 — 응답을 DB·캐시·로그 어디에도 남기지
  않고, 키워드·지역 검색만 중계한다 (좌표 기반 주변검색은 제공하지 않는다, docs/15 §4).
- 방문 관광지(record_places) API는 **측위가 개입하지 않는 경로에만** 열려 있다 (v1.3.0 부터).
  근거는 docs/16-visit-region-api-design.md §9 — 이 표를 채우는 주체가 웹뿐이고, 웹은 키워드 검색만
  제공하므로 저장되는 값이 '이용자로부터 제공받은 정보'다.
  - **좌표를 서버로 들이는 변경은 이 판단을 무효로 만든다.** 브라우저 위치 권한, 좌표를 받는
    프록시 라우트, 요청 스키마의 `.strict()` 해제, 모바일의 GPS 후보 서버 저장이 여기 해당한다.
    코드보다 docs/12-location-law.md §3 재검토가 먼저다.
  - 기록 콘텐츠(제목·일기·해시태그)와 EXIF 제거 사진 API는 검토 대상이 아니다 —
