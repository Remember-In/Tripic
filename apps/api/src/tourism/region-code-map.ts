import { generatedRegionCodeMap } from "@/tourism/region-code-map.data";

interface RawKtoRegionCodes {
  areaCode?: string;
...
export interface ResolvedKtoRegionCodes {
  areaCode: string;
  legalAreaCode?: string;
  legalSigunguCode?: string;
  sigunguCode?: string;
...
// 모바일 사본은 JSON 을 `as unknown as` 로 좁혔지만, 여기서는 데이터 모듈이
// 이미 GeneratedRegionCodeMap 으로 선언돼 있어 단언 없이 그대로 쓴다.
const codeMap = generatedRegionCodeMap;
...
for (const [administrativeCode, legacyCodes] of Object.entries(
  codeMap.administrativeCodeToLegacy,
)) {
...
/**
 * KorService2 의 신형 법정동 코드 응답을 앱이 저장해 온 구형 TourAPI 코드로 정규화한다.
 * 두 체계가 함께 오면 경계 데이터와 직접 연결되는 신형 코드를 우선하고,
 * 아직 매핑되지 않은 개편 지역은 구형 필드로 안전하게 폴백한다.
 */
