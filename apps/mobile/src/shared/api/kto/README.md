...
- 주변 검색 반경과 후보 개수는 서버의 `/app-config` 값을 사용하며, 설정을 불러오지 못하면 앱에 정의된 안전한 기본값으로 동작한다. 과도한 응답으로 기기 메모리에 부담을 주지 않도록 후보는 최대 50개로 제한한다.
- 주변 검색 반경은 TourAPI 운영 범위에 맞춰 1m에서 20km 사이의 정수로 제한한다.
- `EXPO_PUBLIC_KTO_SERVICE_KEY`에는 공공데이터포털에서 발급한 일반 인증키의 디코딩 값을 권장하며, 인코딩 값을 넣어도 한 번 디코딩한 뒤 안전하게 쿼리에 포함한다.
