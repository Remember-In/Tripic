...
| 항목        | 내용                                                                                                           |
| ----------- | -------------------------------------------------------------------------------------------------------------- |
| 상태        | 구현 완료 (v1.3.0) — **웹의 키워드 검색 경로에 한해** 노출 (§9)                                                |
...
> **이 API 를 여는 근거는 "측위가 개입하지 않는다" 하나다 (§9).**
> 웹에 브라우저 위치 권한이나 좌표 기반 주변검색을 붙이는 순간 그 전제가 깨지고 이 판단은 무효가 된다.
> 그런 기능을 검토한다면 코드보다 [12-location-law.md](./12-location-law.md) §3 재검토가 먼저다.
...
6. 개인정보 처리방침에 계정 연결 방문 이력 처리가 반영된다 (§9.4 — 노출의 차단 조건은 아니다).
...
> **이 작업에는 DB 마이그레이션이 있다** — `20260920120000_add_record_place_visit_unique` (v1.3.0).
> `visitedAt` 을 `DATE` 로 좁히고 `@@unique([recordId, ktoContentId, visitedAt])` 를 추가한다.
>
> v1.2.0·v1.2.1 은 마이그레이션이 없어 롤백이 이미지 교체로 끝났지만 이번은 다르다.
> 순서는 Northflank 템플릿이 보장한다(ManualJob → Condition → DeploymentService) —
> **전제로 api 서비스의 auto-deploy 가 꺼져 있어야 한다** (.github/workflows/server-publish.yml 주석).
> 되돌릴 때는 인덱스를 드롭하고 컬럼을 `TIMESTAMP` 로 되돌린다.
...
원래 이 문서는 "위치정보지원센터 사전 검토 통과 전까지 노출 금지" 게이트를 걸고 있었다.
그 게이트의 실질적 근거를 다시 확인한 결과, **웹 경로에는 해당하지 않는다**고 판단해 열었다.

### 9.1 남아 있던 유일한 우려

[12-location-law.md](./12-location-law.md) §3 의 ③번(`record_places` 서버 저장) 판단은 이미
**"위치정보 아닐 가능성 높음"** 이었고, 유보는 하나뿐이었다:

> 다만 후보 목록이 측위값(①)에서 파생된 사슬이라는 보수적 해석 여지

### 9.2 웹 경로에는 그 사슬이 없다

| 확인                               | 결과                                                         |
| ---------------------------------- | ------------------------------------------------------------ |
| 웹의 위치 관련 코드                | 없음 — `geolocation`·`getCurrentPosition`·좌표 필드 0건      |
| 좌표 기반 주변검색 라우트          | 만들지 않음 — `/tourism/nearby` 는 404 (docs/15 §4.2)        |
| 요청 스키마의 좌표 필드            | `.strict()` 로 거부 (§4.2)                                   |
| 모바일이 `record_places` 에 쓰는가 | **아니오** — 모바일의 서버 호출은 `auth/*` 와 `/users/me` 뿐 |

즉 `record_places` 에 행을 만드는 주체는 웹뿐이고, 웹은 **사용자가 검색창에 직접 입력해 고른 값**만
보낸다. 측위가 어디에도 개입하지 않으므로, 해설서 p.20 이 위치정보에서 제외하는
_"이용자로부터 제공받은 정보"_ 에 그대로 해당한다.

모바일의 GPS 주변검색(②)은 앱 안에서 단말→KTO 직접 호출로 끝나고 결과가 서버로 오지 않는다.

### 9.3 이 판단이 무효가 되는 조건

아래 중 **하나라도 생기면** 9.2 의 전제가 깨진다. 코드보다 docs/12 §3 재검토가 먼저다.

- 웹에 브라우저 위치 권한 요청이나 현재 위치 기반 추천을 붙인다
- `/tourism/nearby` 등 좌표를 받는 프록시 라우트를 연다
- 모바일이 GPS 후보에서 확정한 관광지를 서버에 쓰기 시작한다
- 요청 스키마에서 `.strict()` 를 걷어낸다

### 9.4 남은 일 (노출과 무관하게 필요)

- [ ] 위치정보지원센터 사전 상담 — **차단 조건이 아니라 확인 절차**로 남긴다
      (docs/12 자체가 "법률 자문이 아니라 개발팀 판단"이라고 명시한다)
- [ ] 개인정보 처리방침에 계정 연결 방문 이력의 처리 목적·보관·삭제 반영 — 기획 담당
- [ ] 위치기반서비스 이용약관 필요 여부 확정
- [x] `@@unique([recordId, ktoContentId, visitedAt])` 마이그레이션 (v1.3.0, §3.1)
- [x] `apps/api/CLAUDE.md` 금지 규칙 갱신
