...
>
> 개인정보 표현 정정(2026-08-06): 본문에서 “GPS를 앱 내부에서만 사용”하거나 “서버로 전송하지 않는다”는 표현은 Tripic 백엔드·로컬 DB에 저장하지 않는다는 뜻입니다. 위치 기반 후보 조회 시 GPS 좌표는 앱에서 한국관광공사 OpenAPI로 직접 전송됩니다. 현재 기준은 [개인정보·위치정보 정책](./08-privacy-risk.md)과 [배포 전 체크리스트](./legal/release-legal-checklist.md)를 따릅니다.
